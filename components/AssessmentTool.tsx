...
export default function AssessmentTool({ onBookingClick, onAssessmentComplete }: AssessmentToolProps) {
  ...
  const questions = [
    {
      ...
      options: [
        ...
        { value: 'acnescars', label: 'Acne scars', icon: '🌟' },
        { value: 'sundamage', label: 'Sun damage & pigmentation', icon: '☀️' },
        { value: 'texture', label: 'Uneven skin texture', icon: '💎' },
        { value: 'fullface', label: 'Overall skin rejuvenation', icon: '🌸' },
        { value: 'prevention', label: 'Preventative anti-aging', icon: '⏰' }
      ]
    },
    {
      id: 2,
      question: "How would you describe your skin concern severity?",
      options: [
        { value: 'mild', label: 'Mild - Early signs', icon: '🌱' },
        { value: 'moderate', label: 'Moderate - Noticeable', icon: '🌿' },
        { value: 'significant', label: 'Significant - Advanced', icon: '🌳' }
      ]
    },
    {
      id: 3,
  ...
  const getRecommendation = (assessmentAnswers: any = answers) => {
    ...
    if (concern === 'neck' || laxity === 'significant' || (concern === 'fullface' && ageRange === '55+')) {
      return {
        treatment: 'Full Face & Neck CO2 Laser',
        price: '£TBC',
        description: 'Complete face and neck resurfacing for maximum transformation. Targets wrinkles, sun damage, and skin texture for comprehensive rejuvenation.',
        isSuitable: true
      }
    }

    // Full Face CO2 Laser for most cases
    if (concern === 'jowls' || concern === 'cheeks' || concern === 'fullface' || laxity === 'moderate') {
      return {
        treatment: 'Full Face CO2 Laser',
        price: '£TBC',
        description: 'Our comprehensive treatment for full facial resurfacing. Reduces wrinkles, improves skin texture, and reverses sun damage for dramatic results.',
        isSuitable: true
      }
    }
    ...
    if (concern === 'brows' || concern === 'prevention' || laxity === 'mild') {
      return {
        treatment: 'Targeted CO2 Laser',
        price: '£TBC',
        description: 'Perfect for specific areas like crow\'s feet, smoker\'s lines, or forehead. Focused treatment for your main concern with excellent results.',
        isSuitable: true
      }
    }

    // Default to Targeted CO2 Laser
    return {
      treatment: 'Targeted CO2 Laser',
      price: '£TBC',
      description: 'Our introductory offer - perfect for targeting specific areas. An excellent first treatment to experience CO2 Laser results.',
      isSuitable: true
    }
  }
          ...
          {step <= totalSteps && currentQuestion ? (
            ...
            <div className="text-center animate-slide-up">
              ...
              <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4">
                Your Perfect CO2 Laser Treatment
              </h3>
              ...
              <div className="bg-green-50 border-2 border-green-200 rounded-xl p-4 mb-6 text-left">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-green-900 font-medium mb-1">Suitable for Most Skin Types</p>
                    <p className="text-sm text-green-800">
                      CO2 Laser is suitable for most skin types. Jazz will assess your suitability during consultation.
                    </p>
                  </div>
                </div>
              </div>
